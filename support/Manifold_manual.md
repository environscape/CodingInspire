...
#### Manifold的操作逻辑基于12个并列的调制模块与对应的参数，您可以自由路由这些模块之间的调制关系以及输入输出接口。
...
**单击**：用于切换路由模式(Router View)和参数模式(Param View)。在这两个模式中，p1/p2/p3都有不同的作用，他们通常可以对应修改屏幕下方三处的信息
...
每个参数第一行文字表示该接口调制模块内的什么参数，这在不同模块下，对应的参数各不相同，您可以查阅第二章对于模块参数的说明。  
...
预设界面包含一系列工厂预设，您可以直接调用艺术家预设,以及一些常用的预设，方便您快速进入一些特定的调制场景。敬请期待更多的工厂预设。  
