...
<center><H2>Manifold集成式数字调制器 用户手册</H2></center>
...
**当图标处于倒三角时:** 表示您当前调整的模块在十二个模块中的位置；  

**当图标显示一条横线时:** 表示这里还没有设置模块，为空模块；  

**当图标显示一个向右的箭头时:** 表示该模块为输出模块，它被指定到了Out1-6的接口上进行输出信号；  

**当图标显示一个实心圆点时:** 表示该模块已经被创建，但是仅作为内部调制使用，没有指定到输出接口；  
