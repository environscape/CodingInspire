...
每个CV接口都可以设置为4种类型，每个CV接口都可以指定该信息的MIDI通道（为1-16）：

1. **CONTROL_CHANGE** CC信息
2. **NOTE_NUMBER** 音符信息（相当于模块中的VOCT）
3. **NOTE_ON_OFF** 音符开启关闭信息（相当于Gate，控制您配置的音符信息是否启动）
4. **NOTE_VEL** 音符力度信息（控制您配置的音符的力度）

以上信息您需要使用 "四、MIDI参数配置" 使用助手来修改您模块的映射配置，您也可以在购买时让商家协助您配置。

**NOTE_NUMBER、NOTE_ON_OFF、NOTE_VEL** 需要设置第三个参数来进行关联绑定（0-64的数字，当一样时，被认为是一组信息），这样就可以确定是哪一个NOTE_NUMBER与NOTE_ON_OFF、NOTE_VEL是相关的。

**输入范围**：支持 0-5V CV 信号，内置保护电路，超出范围的信号将进行简单削波处理（以限制在有效范围内）。
...
1. 将模块USB连接至您的PC
2. 界面上选择您模块所在的串口端口（如果有多个选择，需要判断哪一个才是您的模块），然后点击"连接模块"
