...
<center><H1>Manifold集成式数字调制器 用户手册</H1></center>
<center>软件版本 v1.10 文档更新日期 2026-08-14</center>


## 一、快速入门

Manifold的操作逻辑基于12个并列的内部调制模块与对应的参数；  
您可以自由路由这些内部模块之间的调制关系以及物理输入输出接口；  
每个内部模块包含三个参数，两个输入，一个输出。  

<br>
<figure>
<img src="../img/Manifold_Structure.png" alt="" width="640" height="800">
    <figcaption>模块简介</figcaption>
</figure>

<figure>
<img src="../img/Manifold_Quickstart.png" alt="" width="960" height="480">
...
<br>
<figure>
<img src="../img/Manifold_Appearance.png" alt="" width="240" height="640">
...
###### 直接旋转
用于切换模块，一共包含十二个模块，您可以根据顶部的指示器(请见8. 指示器 说明)来了解当前处于的模块位置。有些模块一开始是空模块，需要您指定他的类型(请见 11. 屏幕-Router View 说明)

###### 单击
用于切换路由界面(Router View)和参数界面(Param View)。在这两个界面中，P1/P2/P3都有不同的作用，他们通常可以对应修改屏幕下方三处的信息

###### 长按
用于进入预设界面(Preset)，您可以在这个界面的列表中进行预设的保存与读取，也可以进行初始化，以及随机生成预设(请见13. 预设界面说明)。

### 2. P1/In1 旋钮  

###### 直接旋转   
通常用于修改屏幕左下方对应的参数，在路由界面下，旋转则可以改变该路由输入的衰减程度；在参数界面下，选择则可改变对应的参数

###### 单击   
路由界面下，单击后，您可以修改In1接口的路由在参数界面下

###### 按下旋转   
参数界面下，您可以按下同时旋转来快速调整参数
...
###### 直接旋转   
通常用于修改屏幕左下方对应的参数，在路由界面下，直接旋转并不改变任何参数；在参数界面下，选择则可改变对应的参数

###### 单击第1次   
路由界面下，单击第1次后，您可以修改当前模块类型

###### 单击第2次   
路由界面下，单击第2次后，您可以修改当前模块的输出路由
...
<br>
<figure>
    <img src="../img/Manifold_Screen1.png" alt="屏幕主界面" width="280" height="160">
...
###### 当图标处于倒三角时   
表示您当前调整的模块位于十二个模块中的位置  

###### 当图标显示一条横线时   
表示这里还没有设置模块，为空模块  

###### 当图标显示一个向右的箭头时   
表示该模块为输出模块，它被指定到了Out1-6的接口上进行输出信号  

###### 当图标显示一个实心圆点时   
表示该模块已经被创建，但是仅作为内部调制使用，没有指定到输出接口  
...
###### 基本逻辑   
- 由于内部包含十二个模块，每个模块并没有直接相连，需要您在此界面进行模块虚拟跳线的路由，因此它们位于十二个模块中的顺序无关紧要。  
- 当您通过单击<>旋钮并且处于路由界面时，左下方，中下方，右下方将分别展示当前模块In1链接的接口，In2链接的接口，out输出的接口，
并且以第二行文字的形式来告知您该接口在模块上的作用。  
  
###### 设置当前模块的输入路由   
- 左下方/中下方的两个参数，每个参数的第一行文字代表当前模块的输入接口路由到了何处，如果未链接任何接口，则展示"---"，  
...
###### 设置当前模块的类型   
- 对于右下方参数(对应P3旋钮)，P3旋钮则用来修改当前模块的类型，您需要单击P3，屏幕此时会高亮起P3对应的类型，此时旋转P3，您可以修改当前模块的类型。  
...
###### 回到P3旋钮初始状态   
- 最后再次点击P3时，P3将循环回到初始状态，此时它不修改任何参数，以防止您可能误旋转导致模块被轻易改变类型或者被改变输出的路由。  

<br>
<figure>
    <img src="../img/Manifold_Screen2.png" alt="屏幕主界面" width="280" height="160">
...
当您通过长按<>旋钮并且处于预设界面时，您可以通过其他旋钮的按下或旋转来触发预设界面下方的对应功能，包含预设的选择、保存和读取。  
...
###### 进入设置界面   
在预设界面下，屏幕底部显示有四个功能提示：back(<>旋钮)、set(P1旋钮)、save(P2旋钮)、load(P3旋钮)。  
单击P1旋钮(对应"set"提示)，您将进入设置界面(Setting View)，用于调整Manifold的全局参数(请见14. 屏幕-Setting View 设置界面 说明)。

### 14. 屏幕-Setting View 设置界面  

设置界面用于调整Manifold的全局参数，这些参数独立于预设之外，将保存在EEPROM中并在每次开机时自动加载。  

###### 界面布局与操作逻辑   
设置界面与预设界面风格一致，屏幕中央以列表形式展示设置项，当前选中的项目以反白高亮显示，同时显示其上下相邻的项目以便您快速浏览。  
- <>旋钮用于上下翻动，选择需要调整的设置项。  
- P3旋钮用于修改当前选中设置项的数值，对于数值类参数(如Qunt Scale)，旋转P3将以较小步进精确调整；对于列表类参数(如Def Router)，旋转P3将逐个切换选项。  
- P1旋钮与P2旋钮在此界面下不参与修改，P2旋钮用于保存退出(见下文)。  
- 屏幕底部显示按钮提示：back(<>旋钮，不保存退出)、----(P1旋钮，无功能)、save(P2旋钮，保存退出)、----(P3旋钮，无功能)。  

###### 退出与保存   
- 单击<>旋钮(对应"back"提示)：不保存退出，所有修改将被放弃并恢复EEPROM中的原值，返回路由界面。  
- 单击P2旋钮(对应"save"提示)：保存退出，将当前所有设置项的数值写入EEPROM并返回路由界面。  
- 上述两种方式都将退出至路由界面(Router View)。  

###### 设置项详解   

**Qunt Scale**：量化器(QUNT)模块的VOCT输出校准倍率，默认值1.000。  
当您的量化器输出音高与外部设备存在偏差时，可以通过微调此参数来校准VOCT电压。该参数对量化器模块实时生效，修改后无需重新加载预设。  

**Def Router1**：模块初始化时输入1(In1)的默认路由。默认为Gate1。  
可选范围为1MD-12MD(十二个内部模块)、Gate1-3、CV1-3、以及----(无路由)。  
当您执行初始化(Init Preset)操作时，所有模块的In1将自动路由到此设置指定的接口。若该默认路由指向某个内部模块且恰好与当前模块相同，程序将自动避开自环并改路由至Gate1。  

**Def Router2**：模块初始化时输入2(In2)的默认路由。默认为CV1。  
可选范围与Def Router1一致。  
当您执行初始化(Init Preset)操作时，所有模块的In2将自动路由到此设置指定的接口，同样具备自环避让逻辑。  

**Boot Select**：开机时自动加载的预设。默认为Init(不加载任何预设，保持初始化状态)。  
可选范围为：  
1.Init：初始化状态，不加载预设  
2.User 1-16：开机自动加载对应的用户预设  
3.FTY 1-16：开机自动加载对应的工厂预设  
4.Gen：开机时随机生成一组预设  
设置此项并保存后，下次开机时Manifold将自动加载您指定的预设，无需手动操作。  

<br>
<figure>
<img src="../img/Manifold_Workflow.png" alt="" width="960" height="640">
...
**该模块根据BPM计算十六分音符的时间间隔并持续输出时钟脉冲。播放时矩阵会以四个圆点展示当前十六分音符位置，当前位置的圆点为实心；参数界面下State参数会显示当前的小节与拍数（如1/1），支持Gate输入或手动控制启停**

##### 路由设置

**Gate** ：该接口将接受高电平以启动节拍播放，低电平以停止播放  
**RST** ：该接口将接受上升沿以重置播放位置至第一小节第一拍

##### 参数设置

**BPM**：节拍速度，范围50-180  
**---**：  
**State**：手动控制播放状态，参数值较高时启动播放，较低时停止播放(注意：手动启动只接收手动停止 gate启动则只接收gate停止)
...
请在usb连接您的计算机之前按住Pico RP2040的Boot键，然后连接usb与计算机，此时您会在您的计算机中发现一个新的磁盘，此时将下载好的UF2固件文件拖入其中，等待一分钟后，直接移除usb，您的Manifold模块就已经升级完成  