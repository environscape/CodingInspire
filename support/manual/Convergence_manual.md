...
Convergence 会为每一个效果保存各自的参数设置，以便您快速回归先前的效果设置。

**自动保存机制：**

- 当您在参数编辑模式下调整任意参数（参数1、参数2、参数3）后，退出参数编辑模式时，系统会自动保存当前效果的参数配置。
- 保存触发条件：从参数编辑模式（光标框位于参数行）切换回浏览模式（光标框位于 `< >` 图标）时触发保存。

**存储特性：**

- 每个效果的三个参数值均独立保存，互不影响。
- 参数值在设备断电后依然保留，下次开机时自动恢复上次的设置。
- 首次使用时，所有效果的参数默认值为中间位置（50%）。

**操作示例：**

1. 在浏览模式下选择效果 "Shimmer"
2. 短按旋钮进入参数1编辑模式，旋转调整参数值
3. 再次短按旋钮进入参数2编辑模式，旋转调整参数值
4. 继续短按旋钮直到回到浏览模式，此时系统自动保存当前效果的所有参数设置
