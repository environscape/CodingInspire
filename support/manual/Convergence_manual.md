...
Convergence 是一款高性能 Eurorack 格式数字效果处理器模块，集成了 24 种精心设计的音频效果，涵盖调制、延迟和混响三大类。模块采用 FV-1 数字信号处理器芯片，提供专业级的音频处理能力。
...
**技术说明**：该效果模拟经典的 RE301 弹簧混响单元，包含两个延迟磁头（Head 2 和 Head 3），带有 Wow & Flutter 效果模拟磁带机的速度波动。
...
**技术说明**：采用 Jon Dattorro 的 "Effect Design" 论文算法，包含输入扩散网络和双回路混响结构，支持音调偏移产生 Shimmer 效果。
...
**技术说明**：采用左右独立的双回路结构，每个通道包含 3 个可变扩散全通滤波器和 1 个固定系数全通滤波器。
...
**技术说明**：包含 100ms 预延迟缓冲区和多层初始延迟结构，通过多个抽头产生丰富的早期反射声。
...
**技术说明**：在 Dattorro 混响结构中加入固定 +1 八度的音调偏移，产生类似 Shimmer 的彩虹般音色。
...
**技术说明**：将混响输出进行 +1 八度音调偏移后反馈回输入，创造出持续上升的高频泛音效果，常用于氛围音乐制作。
...
**技术说明**：结合了 22938 采样点的长延迟和双回路混响结构，回声滤波器系数根据反馈量动态调整。
...
**技术说明**：使用三个不同长度的延迟线（0、5242、11792 采样点）模拟房间的早期反射声。
...
**技术说明**：基于指数映射的延迟时间控制，包含高通滤波器防止低频堆积，以及软削波限幅器防止过载。
...
**技术说明**：结合了 20000 采样点的延迟线和双回路混响结构，回声输出经过低通滤波处理。
...
**技术说明**：使用双延迟线结构，左声道输出反馈到右声道输入，右声道输出反馈到左声道输入，创造左右交替的回声效果。带有 Wow & Flutter 效果模拟磁带机特性。
...
**技术说明**：包含 LFO 调制的延迟时间变化，结合 SVF 滤波器产生类似星场闪烁的效果。
...
**技术说明**：支持录制模式和播放模式切换，切换时带有淡入淡出过渡防止爆音。采用双抽头乒乓结构，带有固定阻尼滤波。
...
**技术说明**：在标准乒乓延迟基础上加入了 LFO 调制的延迟时间抖动（Wonkiness），创造不稳定的随机效果。
...
**技术说明**：结合了 LFO 调制的滤波器和短延迟，模拟经典模拟移相器的声音特性。
...
**技术说明**：模拟老式油桶延迟（Oil Can Delay）效果，带有调制的延迟时间变化，创造温暖的复古音色。
...
需要拆卸后置主板，接上usb，使用xLoader升级固件。升级文件为hex